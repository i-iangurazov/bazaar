...
const tlsDirectory = await mkdtemp(join(tmpdir(), "bazaar-reporting-tls-"));
const env = {
  ...reportingEnvironment(),

  REPORTING_HTTPS: "1",
  REPORTING_TLS_KEY_PATH: join(tlsDirectory, "localhost.key"),
  REPORTING_TLS_CERT_PATH: join(tlsDirectory, "localhost.crt"),
};
...
await run(process.execPath, ["--import", "tsx", "scripts/reporting/seed.ts"]);
// Compile once: the reporting visual matrix must use the release runtime,
// without retaining every page's development compiler in the browser CI job.
await run(process.execPath, ["--import", "tsx", "scripts/reporting/build.ts"]);
await run("openssl", [
  "req",
  "-x509",
  "-newkey",
  ...
  "-nodes",
  "-keyout",
  env.REPORTING_TLS_KEY_PATH,
  "-out",
  env.REPORTING_TLS_CERT_PATH,
  "-days",
  "1",
  "-subj",
  "/CN=localhost",
...
const certificate = await readFile(env.REPORTING_TLS_CERT_PATH);
await mkdir("artifacts/reporting", { recursive: true });
const log = await open("artifacts/reporting/server.log", "w", 0o600);
const server = spawn(
  process.execPath,
  ["--import", "tsx", "scripts/reporting/dev.ts", "--production"],
  {
  ...
  for (let attempt = 0; attempt < 120; attempt += 1) {
    ...
    try {
      const healthy = await new Promise<boolean>((resolve) => {
        ...
        const request = get(
          "https://127.0.0.1:3123/api/auth/csrf",
          { ca: certificate },
          (response) => {
            response.resume();
