...
describeDb("pos", () => {
  ...
  it("allows POS completion to drive stock negative regardless of the store stock policy", async () => {
    const { org, store, product, cashierUser } = await seedBase({ plan: "BUSINESS" });
  ...
  it("allows a completed-sale quantity increase when restricted stock is exhausted", async () => {
    ...
    expect(payments).toHaveLength(1);
    expect(Number(payments[0]?.amountKgs)).toBe(200);
