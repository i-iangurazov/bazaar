...
const totals = {
  ...
  returnCount: 0,
  discountKgs: 0,
  averageReceiptKgs: 321,
  marginPercent: 68.85,
  markupPercent: 221,
  coveragePercent: 100,
  amountConflictDocuments: 0,
};
const report = (items: unknown[] = [], total = 0) => ({
  series: [],
  totals,
  previous: totals,
  items,
  ...
  period: {
    ...
    previousDateFrom: "2024-02-27",
    previousDateTo: "2024-02-28",
    partial: false,
  },
  meta: { generatedAt: new Date("2024-03-02T00:00:00Z") },
});
...
beforeEach(() => {
  ...
  mocks.registers.mockReturnValue(queryResult([]));
  mocks.cashiers.mockReturnValue(queryResult([]));
  mocks.exportProducts.mockReset();
...
describe("all-filtered report export behavior", () => {
  ...
  it("requests all current filters without pagination and downloads off-page rows", async () => {
    loaded();
    const rows = Array.from({ length: 26 }, (_, index) => product(index));
    mocks.exportProducts.mockResolvedValue(report(rows, 26));
    render(<AnalyticsPage />);
    fireEvent.change(screen.getByLabelText("analytics.filters.productSearch"), {
    ...
    expect(file.fileNameBase).toBe("sales-products-all-filtered-2024-02-29-2024-03-01");
    expect(file.rows).toHaveLength(28);
    expect(file.rows[25][0]).toBe("Product 25");
    expect(file.header).not.toContain("stockRemaining");
  });

  it("shows a row-limit failure without downloading a truncated file", async () => {
    loaded();
    mocks.exportProducts.mockRejectedValue(Error("analyticsExportRowLimit"));
    render(<AnalyticsPage />);
    fireEvent.click(screen.getByRole("button", { name: "analytics.actions.exportAllProducts" }));
    await waitFor(() =>
      expect(screen.getByRole("alert").textContent).toBe("errors.analyticsExportRowLimit"),
    );
    expect(mocks.download).not.toHaveBeenCalled();
  });

  it.each(["filters", "audience", "unmount"])(
    "discards an in-flight export after %s changes",
    async (change) => {
      loaded();
      let complete!: (value: unknown) => void;
      mocks.exportProducts.mockReturnValue(
        new Promise((resolve) => {
          complete = resolve;
        }),
      );
      ...
      expect(
        screen
          .getByRole("button", { name: "analytics.actions.exportingProducts" })
      ...
      if (change === "filters")
        ...
        mocks.session.mockReturnValue({
          status: "authenticated",
          data: { user: { id: "other-actor", organizationId: "other-org", role: "MANAGER" } },
        });
        view.rerender(<AnalyticsPage />);
      } else view.unmount();
      await act(async () => {
        complete(report([product(0)], 1));
      });
      expect(mocks.download).not.toHaveBeenCalled();
    },
  );
...
describe("analytics report URL contract", () => {
  it("builds and round-trips real business dates and a concrete store", () => {
    expect(buildAnalyticsReportHref(input)).toBe(
      "/reports/analytics?dateFrom=2024-02-29&dateTo=2024-03-01&storeId=store-a",
    );
    expect(parse(buildAnalyticsReportHref(input).split("?")[1])).toEqual({
      kind: "valid",
      scope: input,
    });
  });
  ...
  it("accepts366 inclusive days but rejects367", () => {
    expect(() =>
      buildAnalyticsReportHref({ dateFrom: "2024-01-01", dateTo: "2024-12-31" }),
    ).not.toThrow();
    expect(() =>
      buildAnalyticsReportHref({ dateFrom: "2024-01-01", dateTo: "2025-01-01" }),
    ).toThrow("invalidInput");
  });
  it.each([
    "dateFrom=2026-02-29&dateTo=2026-03-01",
    "dateFrom=2026-04-31&dateTo=2026-05-01",
    "dateFrom=2026-01-02&dateTo=2026-01-01",
    "dateFrom=2026-1-01&dateTo=2026-01-02",
    "dateFrom=2026-01-01",
    "dateTo=2026-01-02",
    "storeId=store-a",
    "dateFrom=&dateTo=2026-01-02",
    "dateFrom=2026-01-01&dateTo=2026-01-02&storeId=",
...
describe("report scope behavior", () => {
  it("waits for fresh authorized stores and never fetches the default period for a deep link", () => {
    ...
    expect((screen.getByLabelText("analytics.filters.dateTo") as HTMLInputElement).value).toBe(
      input.dateTo,
    );
  ...
  it("rejects an unauthorized store without silently broadening to all stores or rendering results", () => {
    ...
    for (const query of queries()) expect(active(query)).toHaveLength(0);
    expect(screen.queryByText("reporting.netSales")).toBeNull();
  });
  it.each([
    "dateFrom=2026-04-31&dateTo=2026-05-01",
    "dateFrom=2026-01-01&dateFrom=2026-01-02&dateTo=2026-01-03",
  ])("blocks all report queries for invalid URL %s", (query) => {
    mocks.query = query;
    render(<AnalyticsPage />);
  ...
  it("uses the newly navigated scope atomically and reapplies the old scope on back navigation", () => {
    const first = buildAnalyticsReportHref(input),
      next = buildAnalyticsReportHref({
        dateFrom: "2026-08-01",
        dateTo: "2026-08-04",
        storeId: "store-b",
      });
    ...
    for (const href of [next, first]) {
      ...
      setUrl(href);
      view.rerender(<AnalyticsPage />);
      const expected = parse(href.split("?")[1]);
      expect(expected.kind).toBe("valid");
  ...
  it("writes manual dates/store selection to the URL and restores them on refresh", () => {
    setUrl(buildAnalyticsReportHref(input));
    const view = render(<AnalyticsPage />);
    fireEvent.change(screen.getByLabelText("analytics.filters.dateTo"), {
    ...
    view.rerender(<AnalyticsPage />);
    fireEvent.change(screen.getByLabelText("analytics.filters.store"), {
      target: { value: "all" },
    });
    expect(mocks.replace).toHaveBeenLastCalledWith(
    ...
    expect(mocks.overview.mock.lastCall?.[0]).toMatchObject({
      dateFrom: "2024-02-29",
      dateTo: "2024-03-02",
      storeId: undefined,
    });
    fireEvent.change(screen.getByLabelText("analytics.filters.productSearch"), {
      target: { value: "allowed product" },
    });
    expect(active(mocks.overview).at(-1)?.[0].search).toBe("allowed product");
  });
  it("keeps a cleared manual date explicit and blocks queries until corrected", () => {
    setUrl(buildAnalyticsReportHref(input));
    const view = render(<AnalyticsPage />);
    fireEvent.change(screen.getByLabelText("analytics.filters.dateFrom"), {
      target: { value: "" },
    });
    setUrl(mocks.replace.mock.lastCall![0]);
    queries().forEach((query) => query.mockClear());
    view.rerender(<AnalyticsPage />);
    expect(screen.getByRole("alert").textContent).toBe("errors.invalidInput");
    expect((screen.getByLabelText("analytics.filters.dateFrom") as HTMLInputElement).value).toBe(
      "",
  ...
  it("hides an existing report when its selected store access is revoked", () => {
    setUrl(buildAnalyticsReportHref(input));
    const view = render(<AnalyticsPage />);
    expect(screen.getByText("reporting.netSales")).toBeTruthy();
    mocks.stores.mockReturnValue(queryResult(stores.slice(1)));
    queries().forEach((query) => query.mockClear());
    view.rerender(<AnalyticsPage />);
    expect(screen.getByRole("alert").textContent).toBe("errors.storeAccessDenied");
    expect(screen.queryByText("reporting.netSales")).toBeNull();
