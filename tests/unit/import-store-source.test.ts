...
describe("import store selection source", () => {
  it("keeps product imports targeted to an explicit store", async () => {
    ...
    const importServiceSource = await readSource("src/server/services/imports.ts");

    expect(pageSource).toContain("targetStoreId");
    expect(pageSource).toContain("storeId: targetStoreId");
    expect(pageSource).toContain("!targetStoreId");
    expect(pageSource).toContain('useState<ProductStockBehavior>("set")');
    expect(pageSource).toContain('"в наличии"');
    expect(pageSource).toContain('"цена"');
    expect(pageSource).toContain("applyDetectedProductMapping");
    expect(pageSource).toContain("product-import-template");
    expect(pageSource).not.toContain("template-1c");
    ...
    expect(importServiceSource).toContain('row.stockQty !== undefined) ? "set" : "ignore"');
  });
