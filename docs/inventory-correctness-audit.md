...
- All current physical writers now pass through `applyStockMovement`: snapshot row lock, signed movement and optional lot adjustment in the same transaction. Only zero initialization, incoming-PO aggregates, guarded recomputation and deletion of unused products remain separate. The database increments `InventorySnapshot.version` whenever `onHand` changes, including change-and-change-back (ABA).
...
| Duplicate product / clone store | New opening history; no foreign purchase commitments | `inventory-correctness`: copy stock/lot/history, source PO remains source-only: pass | Clone no longer copies `onOrder`; copied physical stock creates lots |
