...
`pnpm test:ux` применяет штатные миграции к **пустой локальной** `bazaar_hardening_ux`, создаёт синтетические товары, магазины и четыре роли, собирает production-пакет, запускает HTTPS-браузер и сохраняет `artifacts/ux/flows` и `artifacts/ux/after`. Скрипты не принимают production URL. AI/email/payment/storage credentials очищаются; внешние эффекты отключены; браузер блокирует сторонние запросы. Повторный seed в непустую БД намеренно запрещён.
...
- Production build — успешно; на собранном HTTPS-пакете повторены **21 сквозных браузерных сценариев**. Проверены реальные ответы и записи штатных сервисов, а не только видимый текст об успехе.
...
Раннее открытие BAAM при задержке NextAuth теперь показывает загрузку вместо ложного отказа в доступе. Браузер удерживает ответ сессии до клика, проверяет загрузку, затем разрешает штатный ответ и проверяет открытый чат и фокус. STAFF/CASHIER по-прежнему не получают доступ. Прямые ссылки создания магазина и поставщика также ожидают роль до удаления `create=1`; браузер проверяет обе формы с задержанной сессией и фокусом. Полная визуальная матрица запускается на production-пакете; холодная компиляция очередной страницы dev-сервера больше не входит в измеряемый пользовательский сценарий.
