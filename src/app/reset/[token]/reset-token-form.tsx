...
export const ResetTokenForm = ({ token }: { token: string }) => {
  ...
  const submitReset = (values: z.infer<typeof schema>) => {
    if (submitInFlightRef.current) return;
    submitInFlightRef.current = true;
    resetMutation.mutate({ token, ...values });
  };
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-xl flex-col justify-center gap-4 px-4 py-8 sm:py-12">
      ...
      <Card>
        <CardHeader>
          <CardTitle as="h1">{t("resetTitle")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {done ? (
            ...
            <Form {...form}>
              <form noValidate aria-live="polite" onSubmit={form.handleSubmit(submitReset)}>
                <FormStack>
                  <FormField
