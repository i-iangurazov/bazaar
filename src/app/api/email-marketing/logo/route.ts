...
import { upsertEmailMarketingStoreLogo } from "@/server/services/emailMarketing";
...
export const POST = async (request: Request) => {
  ...
  try {
    ...
    const uploaded = await uploadProductImageBuffer({
      ...
      productId: `email-marketing-logo-${storeId}`,
      buffer,
      contentType,
      sourceFileName: file.name,
    });
    const logo = await upsertEmailMarketingStoreLogo({
      user: {
        id: String(token.sub),
        organizationId: String(token.organizationId),
      ...
      requestId: request.headers.get("x-request-id") ?? `email-logo-${Date.now()}`,
      storeId,
      imageUrl: uploaded.url,
    });

    return Response.json({ logo }, { status: 200 });
  } catch (error) {
