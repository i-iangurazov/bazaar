...
type InventoryWriteOffsPageProps = {
  editDocumentKey?: string;
  editBackHref?: string;
};

export const InventoryWriteOffsPage = ({
  editDocumentKey,
  editBackHref = "/inventory/movements",
}: InventoryWriteOffsPageProps = {}) => {
  ...
  useEffect(() => {
    ...
    const documentReason = (editableDocument as { reason?: string | null }).reason;
    setStoreId(editableDocument.storeId);
    setReason(
      documentReason && WRITE_OFF_REASONS.includes(documentReason as StockWriteOffReason)
        ? (documentReason as StockWriteOffReason)
        : "",
    );
    setComment(editableDocument.notes ?? "");
    setSearch("");
    setLines(
  ...
  const pageTitle = isEditMode ? t("writeOffEditTitle") : t("stockWriteOff");
  const pageSubtitle = isEditMode ? t("writeOffEditSubtitle") : t("stockWriteOffSubtitle");
  const backHref = isEditMode ? editBackHref : "/inventory";
  const saving = writeOffMutation.isLoading || editMutation.isLoading;
  const submitLabel = isEditMode ? t("saveChanges") : t("writeOffPost");
  const submitShortLabel = isEditMode ? t("saveChangesShort") : t("writeOffPostShort");
