...
export const getBaamSalesMetrics = async (
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const storeIds = input.storeId ? [input.storeId] : stores.map((store) => store.id);
    const scope = { organizationId: actor.organizationId, storeIds };
    const report = await getSalesAnalyticsOverview({ ...scope, ...range }, tx);
    const totals = report.totals;
    const paymentsKgs = money(Object.values(totals.paymentBreakdown).reduce((sum, value) => sum + value, 0));
    const refundsKgs = money(Object.values(totals.refundBreakdown).reduce((sum, value) => sum + value, 0));
    ...
    const queryHash = createHash("sha256").update(JSON.stringify({
      version: BAAM_METRIC_VERSION, ...scope,
      fromUtc: range.fromUtc.toISOString(), toUtcExclusive: range.toUtcExclusive.toISOString(),
    })).digest("hex");
    return {
      version: BAAM_METRIC_VERSION,
      queryHash,
      audience: { actorId: actor.id },
      scope: { ...scope, availableStores: stores },
      period: {
        dateFrom: range.dateFrom, dateTo: range.dateTo, timeZone: range.timeZone,
        fromUtc: range.fromUtc.toISOString(), toUtcExclusive: range.toUtcExclusive.toISOString(),
      },
      ...
      days: report.series.map((day) => ({
        date: day.date, salesBeforeReturnsKgs: day.grossSalesKgs,
        returnsKgs: day.returnsKgs, netSalesKgs: day.netSalesKgs,
        receiptCount: day.receiptCount, returnCount: day.returnCount,
        recordedDiscountKgs: day.discountKgs,
        averageReceiptKgs: day.receiptCount ? day.averageReceiptKgs : null,
      })),
      freshness: {
        queryStartedAt, queriedAt: new Date().toISOString(),
        cache: "bypassed" as const, isolation: "repeatable_read" as const,
      ...
      quality: {
        ...
        salesDifferenceKgs, refundsDifferenceKgs,
      ...
      policy: {
        ...
        exclusions: ["profit", "tax", "inventory", "all_channel_revenue", "source_completeness"] as const,
      },
    };
